...
/**
 * 
 * Class for all constants which are used to indicate change types. Note that all ChangeType objects must be accessible
 * via a public static field of some class or interface. These should be specified at construction time, so that the
 * ChangeType can be properly serialized. Typically, they should be constructed using code like:
 * 
 * <pre>
 * class MyClassWhichCanFireChangeEvents
 * {
 * 	public final static ChangeType	CHANGE_COLOR	= new ChangeType(&quot;Color change&quot;,
 * 														MyClassWhichCanFireChangeEvents.class, &quot;CHANGE_COLOR&quot;);
 * 	// Rest of the class here...
 * }
 * </pre>
 * 
 * <p>
 * As of BioJava 1.2, the known ChangeTypes of a system follow a simple hierarchy with single inheritance. All
 * ChangeTypes (except ChangeType.UNKNOWN) have a parent ChangeType (defaulting to ChangeType.UNKNOWN). Generally, when
 * a listener is registered for changetype <code>foo</code>, changes of type <code>bar</code> should be accepted if
 * <code>bar</code> is a sub-type of <code>foo</code>. This can be checked using an expression like:
 * </p>
 * 
 * <pre>
...
public final class ChangeType implements Serializable
{
	 ...
	 * do not use this when there is another, more sensible, option. This is the fallback for when you realy don't know
	 * what else to do.
	 * 
	 * <p>
	 * As of BioJava 1.2, this type is the root of the ChangeType hierarchy. Listening for this type is equivalent to
	 * listening for all ChangeTypes.
	 * </p>
	 */
	 ...
	 * @param fname The name of the field in <code>clazz</code> which is to contain a reference to this change.
	...
	public ChangeType(String name, String className, String fieldName, ChangeType superType) {
		...
		try {
			...
			throw new AssertionFailure("Couldn't find class or field " + className + "->" + fieldName, ex);
		}
	}
	...
	/**
	 * Get all ChangeType objects defined within a class. This includes ChangeTypes defined in superclasses and
	 * interfaces. Only fields declared as public [final] static ChangeType are returned.
	 * 
	...
	/**
	 * Return <code>true</code> iff <code>ct</code> is equal to this type or any of it's supertypes (including
	 * ChangeType.UNKNOWN). If this is true, then ct is more general than this.
	 * 
	 * @since 1.2
	 */
