...
public class SimpleAnnotationWaysGetterReader implements
		AnnotationWaysGetterReader {
	...
	public AnnotationWaysGetter parse(String loc) throws ParserException {
		...
		while (loc != null && loc.length() != 0) {
				...
				if (loc.charAt(1) == '[') {
					...
					if (posEndClusterName < 0) {
						throw new ParserException(
								"AnnotationClusterName without close ']'. Loc="
										+ loc);
					}
					String annotationClusterName = loc.substring(2,
							posEndClusterName);
					AnnotationClustersGetter annotClustersGetter = clusterRepository
							.getAnnotationClusterGetter(annotationClusterName);
					if (annotClustersGetter == null) {
						throw new ParserException("AnnotationClusterName '"
								+ annotationClusterName + "' not found. Loc="
								+ loc);
					}
					...
					if (changerStr.equalsIgnoreCase("AS")) {
						...
						throw new ParserException(changerStr
								+ " is not a valid changer String. Loc=" + loc);
					}
			...
			case '#':
				...
				if (posEndFilter < 0) {
					throw new ParserException(
							"CompRegex without close filter ')'. Loc=" + loc);
				}
				CompRegex compRegex;
				char compRegexChr = loc.charAt(1);
				if (compRegexChr == '=') {
					compRegex = new CompRegexMatch(loc.substring(2,
							posEndFilter));
				} else if (compRegexChr == '!') {
					compRegex = new CompRegexNotMatch(loc.substring(2,
							posEndFilter));
				} else {
					throw new ParserException(compRegexChr
							+ " is not a valid compRegex comparator. Loc="
							+ loc);
				}
			...
			case '=':
				...
				if (posEndFilter < 0) {
					throw new ParserException(
							"CompNumber without close filter ')'. Loc=" + loc);
				}
				...
				try {
					...
					throw new ParserException(numberStr
							+ " is not a valid number. Loc=" + loc, e);
				}
				...
				case '>':
					...
					break;
				default:
					throw new ParserException(compNumberChr
							+ " is not a valid compNumber comparator. Loc="
							+ loc);
				}
