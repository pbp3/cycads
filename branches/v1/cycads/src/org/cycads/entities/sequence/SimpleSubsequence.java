...
public class SimpleSubsequence<S extends Sequence<?, ?, ?, ?, ?, ?>, X extends Dbxref<?, ?, ?, ?>, T extends Type, M extends AnnotationMethod>
		...
		implements
		Subsequence<S, SimpleSubseqAnnotation<?, ?, ?, ?, ?>, Function, X, T, M> {
	...
	@Override
	public SubseqDbxrefAnnotation<?, ?, ?, ?, ?> addDbxrefAnnotation(M method,
			X dbxref) {
		SubseqDbxrefAnnotation<?, ?, ?, ?, ?> annot;
		Collection<? extends SubseqDbxrefAnnotation<?, ?, ?, ?, ?>> annots = getDbxrefAnnotations(
				method, dbxref);
		if (annots.isEmpty()) {
	...
	@Override
	public SubseqFunctionAnnotation<?, ?, ?, ?, ?> addFunctionAnnotation(
			M method, Function function) {
	...
	@Override
	public SubseqDbxrefAnnotation<?, ?, ?, ?, ?> createDbxrefAnnotation(
			M method, X dbxref) {
	...
	@Override
	public SimpleSubseqFunctionAnnotation<?, ?, ?, ?, ?> createFunctionAnnotation(
			M method, Function function) {
	...
	@Override
	public Collection<? extends SimpleSubseqFunctionAnnotation<?, ?, ?, ?, ?>> getFunctionAnnotations(
			M method, Function function) {
		...
		for (SimpleSubseqFunctionAnnotation<?, ?, ?, ?, ?> annot : functionAnnots) {
			if ((function == null || function.equals(annot.getFunction()))
					&& (method == null || method.equals(annot
							.getAnnotationMethod()))) {
				ret.add(annot);
			}
		}
	...
	public Collection<? extends SimpleSubseqAnnotation<?, ?, ?, ?, ?>> getAnnotations(
			M method, Collection<T> types, X synonym) {
		...
		for (SimpleSubseqDbxrefAnnotation<?, ?, ?, ?, ?> annot : dbxrefAnnots) {
			if ((synonym == null || annot.isSynonym(synonym.getDbName(),
					synonym.getAccession()))
					&& (method == null || method.equals(annot
							.getAnnotationMethod()))) {
				boolean isType = true;
		...
		for (Intron intron : introns) {
			if ((intron.getStart() < start && intron.getEnd() > end)
				...
				if (intron.getStart() < start) {
					intronsToAdd.add(new SimpleIntron(intron.getStart(),
							start - 1));
				}
				if (intron.getEnd() > end) {
					intronsToAdd
							.add(new SimpleIntron(end + 1, intron.getEnd()));
				}
			}
		}
