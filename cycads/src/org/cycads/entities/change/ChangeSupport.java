 ...
 * 
 * <p>
 * For an object to correctly fire these events, they must follow a broad outline like this: <code><pre>
 ...
 * 
 * <p>
 * The methods that delegate adding and removing listeners to a ChangeSupport must take responsibility for synchronizing
 * on the delegate.
 * </p>
 * 
...
public class ChangeSupport<O>
{
	private int								listenerCount;
	private int								delta;
	private Set								unchanging;
	private Reference<ChangeListener<O>>[]	listeners;
	...
	/**
	 * Generate a new ChangeSupport instance which has room for initialSize listeners before it needs to grow any
	 * resources.
	 * 
	 * @param initialSize the number of listeners that can be added before this needs to grow for the first time
	 */
	...
	/**
	 * Generate a new ChangeSupport instance which has room for initialSize listeners before it needs to grow any
	 * resources, and which will grow by delta each time.
	 * 
	 * @param initialSize the number of listeners that can be added before this needs to grow for the first time
	 ...
	 */
	public void addChangeListener(ChangeListener<O> cl) {
	 ...
	 */
	public void addChangeListener(ChangeListener<O> cl, ChangeType ct) {
		if (ct == null) {
		...
		synchronized (this) {
			...
			listeners[listenerCount] = new WeakReference<ChangeListener<O>>(cl);
			listenerCount++;
		}
	}
	...
	protected void growIfNecessary() {
		...
		if (listenerCount == listeners.length) {
			...
			Reference<ChangeListener<O>>[] newList = new Reference[newLength];
	 ...
	 */
	public void removeChangeListener(ChangeListener<O> cl) {
	 ...
	 */
	public void removeChangeListener(ChangeListener<O> cl, ChangeType ct) {
		synchronized (this) {
			for (int i = 0; i < listenerCount; i++) {
	...
	/**
	 * Remove all references to listeners which have been cleared by the garbage collector. This method should only be
	...
	protected void reapGarbageListeners() {
		...
		for (int p = 0; p < listenerCount; ++p) {
			Reference<ChangeListener<O>> r = listeners[p];
			if (r.get() != null) {
				types[pp] = types[p];
				listeners[pp] = r;
				pp++;
	 ...
	 * 
	 * <p>
	 * Listeners will be informed if they were interested in all types of event, or if ce.getType() is equal to the type
	 ...
	 */
	public void firePreChangeEvent(ChangeEvent<O> ce) throws ChangeVetoException {
	...
	public void firePostChangeEvent(ChangeEvent<O> ce) {
